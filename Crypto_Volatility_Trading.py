"""
9시에 큰 변동성이 생기는 것을 이용하여 약간의 수익률만을 목표로 빠르게 매매하는 전략임.
하지만 결국은 수익률이 별로 없는 것으로 판별됨. 
근본 원인은 세력들은 이미 지정가를 걸어두고 급등을 시키기 때문에, 
후속으로 매수에 들어가는 이 전략은 불리할 수 밖에 없다라고 결론지었음.

그리고 거래금액이 조금만 늘어나도 슬리피지 문제가 있고, 호가창 단위가 큰 코인들은 매매하기가 곤란함.
하지만 업비트나 빗썸이 아니라, 바이낸스 선물같이 거래량이 많은 거래소에서 매매한다면 어느정도 해결은 되리라 생각함.
"""

#코드자체는 기본적인 로직들을 포함하므로, 추후에 수정하여 다른 방면으로 확장할 수 있을 것 같다. 
#2025년 초에 작성된 코드로써 다시 1번 정리할 필요가 있다. (2026년 상반기내로 정리 예정.)
